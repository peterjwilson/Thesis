...
Write out strain formulation of above example - show it is massive and far outside the linear range. Practical shell elements need to be accurate, and capture non-linear behaviour, but also need to be fast, which a linearized strain formulation provides. One solution is the use of a co-rotational transformation.

\subsection{General approach}
rigid body motion to minimize displacements

\subsection{Formulation overview}
Different alternatives in the EICR approach exist. Currently the CSE formulation is used. Go through formulation. Warpage correction??
