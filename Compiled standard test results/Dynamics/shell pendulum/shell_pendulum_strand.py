...
time_strand = [0]
strand_shell_pendulum_lumped = [0]
counter = 0
with open("strand_shell_pendulum_lumped.txt", "r") as fol:
  for line in fol:
    counter += 1
    if counter%4 == 0:
        line = line.split(' ')
        ...
        #print(line[0])
        #print(line[1])
        time_strand.append((float(line[2])))    
        strand_shell_pendulum_lumped.append((float(line[3])))   
fol.close()
...
plt.grid()
plt.tick_params(labelsize=labelfontsize)
plt.savefig('shell_pendulum_lumped.pdf',bbox_inches="tight")
