...
tri_thru_tsai_no_in_plane_top = []
tri_thru_tsai_no_in_plane_bot = []
with open("tri_thru_tsai_no_in_plane.txt", "r") as kratos_file:
  for line in kratos_file:
      line = line.split('\t')
      for i in range(6):
          if i < 3:
              tri_thru_tsai_no_in_plane_top.append(float(line[2+i]))   
          else:
              tri_thru_tsai_no_in_plane_bot.append(float(line[2+i]))           
 
              
tri_thru_tsai_with_in_plane_top = []
tri_thru_tsai_with_in_plane_bot = []
with open("tri_thru_tsai_with_in_plane.txt", "r") as kratos_file:
  for line in kratos_file:
      line = line.split('\t')
      for i in range(6):
          if i < 3:
              tri_thru_tsai_with_in_plane_top.append(float(line[2+i]))   
          else:
              tri_thru_tsai_with_in_plane_bot.append(float(line[2+i]))                  
...
ref_thru_z_top =[0.5,0.25]
ref_thru_z_bot =[-0.5,-0.25]
...
plt.plot(ref_thru_s_xz,ref_thru_z_top,color = 'grey', linewidth=3.0, linestyle='--', label = 'Ref',antialiased=True)

plt.plot(ref_thru_s_xz,ref_thru_z_bot,color = 'grey', linewidth=3.0, linestyle='--', label = None,antialiased=True)
...
plt.plot(tri_thru_tsai_no_in_plane_top,kratos_thru_z_tsai_top, color = '#FF91A4', linewidth=3.0, linestyle='--', label = 'DSG*',antialiased=True)

plt.plot(tri_thru_tsai_no_in_plane_bot,kratos_thru_z_tsai_bot, color = '#FF91A4', linewidth=3.0, linestyle='--',label = None,antialiased=True)

plt.plot(tri_thru_tsai_with_in_plane_top,kratos_thru_z_tsai_top, color = '#FF91A4', linewidth=3.0,  label = 'DSG',antialiased=True)

plt.plot(tri_thru_tsai_with_in_plane_bot,kratos_thru_z_tsai_bot, color = '#FF91A4', linewidth=3.0, label = None,antialiased=True)
...
plt.grid()
plt.tick_params(labelsize=labelfontsize)
plt.savefig('navier_plate_composite_tri_tsai.pdf',bbox_inches="tight")
