...
time_kratos = [0]
composite_thin_quad_oscillating_lumped = [0]
with open("composite_thin_quad_oscillating_lumped.txt", "r") as fol:
  for line in fol:
    line = line.split(' ')
    time_kratos.append((float(line[0])))    
    composite_thin_quad_oscillating_lumped.append((float(line[1])))   
fol.close()
...
composite_thick_tri_oscillating_plate_lumped = [0]
with open("composite_thick_tri_oscillating_plate_lumped.txt", "r") as fol:
  for line in fol:
    line = line.split(' ')
    #time_kratos.append((float(line[0])))    
    composite_thick_tri_oscillating_plate_lumped.append((float(line[1])))   
fol.close()
...
time_strand = [0]
strand_composite_oscillating_lumped = [0]
counter = 0
with open("strand_composite_oscillating_lumped.txt", "r") as fol:
  for line in fol:
    counter += 1
    if counter%1 == 0:
        line = line.split('\t')
        time_strand.append((float(line[1])))    
        strand_composite_oscillating_lumped.append((float(line[2])))   
fol.close()
...
fig = plt.figure(2)
plt.plot(time_kratos,composite_thin_quad_oscillating_lumped, color = '#77B5FE', linewidth=2.0, label = 'ANDES-DKQ (Lumped)',antialiased=True)

plt.plot(time_kratos,composite_thick_tri_oscillating_plate_lumped, color = '#FF91A4', linestyle='--', linewidth=2.0, label = 'DSG (Lumped)',antialiased=True)

plt.plot(time_strand,strand_composite_oscillating_lumped,color = 'grey', linestyle='None', markerfacecolor= 'None', markersize = 7.0, marker='o', label = 'Strand7 (Lumped)',antialiased=True)
...
plt.grid()
plt.tick_params(labelsize=labelfontsize)
plt.savefig('composite_oscillating_plate_lumped.pdf',bbox_inches="tight")
